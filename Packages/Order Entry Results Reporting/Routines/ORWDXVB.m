...
GETALL(OROOT,DFN,LOC) ;Get all data in one call and let the GUI divide it up
 ...
 S CNT=CNT+1,^TMP("ORVBECINFO",$J,CNT)="~TYPE AND SCREEN",CNT=CNT+1,^TMP("ORVBECINFO",$J,CNT)="i"_$O(^ORD(101.43,"S.VBEC","TYPE & SCREEN",0))
 ...
 S CNT=CNT+1,^TMP("ORVBECINFO",$J,CNT)="~INFO",I=0
 D PTINFO(.INFO,.ORX)
 F  S I=$O(^TMP("ORVBEC",$J,I)) Q:'I  S CNT=CNT+1,^TMP("ORVBECINFO",$J,CNT)="i"_^TMP("ORVBEC",$J,I,0)
 K ^TMP("ORVBEC",$J)
 Q
