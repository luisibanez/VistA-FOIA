ORWPT	; SLC/KCM/REV - Patient Lookup Functions ;10:42 AM  3 Aug 2009
	;;3.0;ORDER ENTRY/RESULTS REPORTING;**10,85,132,149,206,187,190,215,243,269**;Dec 17, 1997;Build 33;WorldVistA 30-June-08
	;
...
IDINFO(REC,DFN)	; Return identifying information for a patient
	...
	;WorldVistA below add HRN & ALT HRN
	; PID^DOB^SEX^VET^SC%^WARD^RM-BED^NAME^HRN^ALTHRN
	N X0,X1,X101,X3,XV  ; name/dob/sex/ssn, ward, room-bed, sc%, vet
	S X0=$G(^DPT(DFN,0)),X1=$G(^(.1)),X101=$G(^(.101)),X3=$G(^(.3)),XV=$G(^("VET"))
	;Begin WorldVistA change; OR*3*269 ;08/02/2009
	N HRN,ID
	S HRN=$$HRN^DGLBPID(DFN)
	S ID=$$ID^DGLBPID(DFN)
	I (ID=HRN)&(HRN'="") D
	.S REC=U_$$DOB^DPTLK1(DFN,2)_U_$P(X0,U,2)_U_$P(XV,U)_U_$P(X3,U,2)_U_$P(X1,U)_U_$P(X101,U)_U_$P(X0,U)_U_$$HRNRET(DFN)_U_$$ALTHRN^ORWPT2(DFN) ;DG249
	E  D
	.S REC=$$ID^DGLBPID(DFN)_U_$$DOB^DPTLK1(DFN,2)_U_$P(X0,U,2)_U_$P(XV,U)_U_$P(X3,U,2)_U_$P(X1,U)_U_$P(X101,U)_U_$P(X0,U)_U_$$HRNRET(DFN)_U_$$ALTHRN^ORWPT2(DFN) ;DG249
	...
	Q
	;Begin WorldVistA change; OR*3*269 ;08/02/2009
	;Return HRN .CHECK FOR "sensitive" patients
HRNRET(DFN)	;
	;
	N IRET
	S IRET=$$HRN^DGLBPID(DFN) ;$$HRN^VWVOEDPT(DFN)
	;I (IRET'="")&$$SCREEN^DPTLK1(DFN) Q "*SENSITIVE*"  ;"HRN SENSITIVE"
	I (IRET'="") Q "'"_IRET_"'" ;"HRN:"_"'"_IRET_"'"
	Q ""
	;End WorldVistA change
...
SELECT(REC,DFN)	; Selects patient & returns key information
	...
	; 13  14  15  16
	; SC%^ICN^AGE^TS
	;WorldVistA change add HRN & ALTERNATE HRN
	; 13  14  15  16  17  18
	; SC%^ICN^AGE^TS^HRN^AltHRN
	;
	; for CCOW (RV - 2/27/03)  name="-1", location=error message
	I '$D(^DPT(+DFN,0)) S REC="-1^^^^^Patient is unknown to CPRS." Q
	;
	N X,ID,HRN ;WorldVistA change; OR*3*269; 08/02/2009
	K ^TMP("ORWPCE",$J) ; delete PCE 'cache' when switching patients
	D VWPT1^ORWPT2 ;WorldVistA change; OR*3*269; 08/02/2009
	...
	D VWPT2^ORWPT2 ;WorldVistA change; OR*3*269; 08/02/2009
