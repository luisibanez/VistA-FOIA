...
BBITEM(ITEMS,DFN,FMT,OLDEST,NEWEST,CNT,TMP) ; from ORWGAPIR
 ...
 S INEWEST=$$LRIDT^ORWGAPIA(NEWEST),IOLDEST=$$LRIDT^ORWGAPIA(OLDEST)
 S LRDFN=$$LRDFN^ORWGAPIA(DFN)
 S IDATE=0
 ...
 Q
 ;
BBDATA(DATA,ITEM,START,DFN,CNT,TMP) ; from ORWGAPIR
...
BBX(DFN) ; $$(dfn) -> 1 if patient has blood bank data ,else 0
 Q $L($O(^LR(+$$LRDFN^ORWGAPIA($G(DFN)),1.6,"")))>0
 ;
