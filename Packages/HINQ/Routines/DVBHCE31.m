...
1 S DW=".52;5",DV="RSX",DU="",DLB="POW STATUS INDICATED?",DIFLD=.525
 S DE(DW)="C1^DVBHCE31",DE(DW,"INDEX")=1
...
X7 I DVBFL'?3N1" - "1U.E S Y="@27",JP=JP+1
 Q
8 D:$D(DG)>9 F^DIE17,DE S DQ=8,DW=".31;4",DV="*P4'",DU="",DLB="CLAIM FOLDER LOCATION",DIFLD=.314
 S DE(DW)="C8^DVBHCE31"
 S DU="DIC(4,"
 S X=+DVBFL
 S Y=X
...
C8F1 Q
X8 S DIC("S")="I $$GET1^DIQ(4,+Y,99)'=""""" D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X
 Q
 ;
...
X25 I $P(DVBP(6),U,4)[" " S Y="@45",JP=JP+1
 Q
26 D:$D(DG)>9 F^DIE17 G ^DVBHCE32
