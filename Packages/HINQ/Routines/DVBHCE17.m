...
DE S DIE="^DPT(",DIC=DIE,DP=2,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^DPT(DA,""))=""
 I $D(^(.11)) S %Z=^(.11) S %=$P(%Z,U,1) S:%]"" DE(8)=% S %=$P(%Z,U,2) S:%]"" DE(9)=% S %=$P(%Z,U,3) S:%]"" DE(10)=%
 I $D(^(.3)) S %Z=^(.3) S %=$P(%Z,U,11) S:%]"" DE(2)=%
 I $D(^(.362)) S %Z=^(.362) S %=$P(%Z,U,14) S:%]"" DE(1)=% S %=$P(%Z,U,20) S:%]"" DE(3)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
X1 S DFN=DA D MV^DGLOCK
 Q
 ;
2 D:$D(DG)>9 F^DIE17,DE S DQ=2,DW=".3;11",DV="SX",DU="",DLB="RECEIVING VA DISABILITY?",DIFLD=.3025
 S DE(DW)="C2^DVBHCE17"
 S DU="Y:YES;N:NO;U:UNKNOWN;"
 G RE
...
C2F1 Q
X2 S DFN=DA D MV^DGLOCK I $D(X),X="Y" D EC^DGLOCK1
 Q
 ;
...
X7 S DVBJ2=1
 Q
8 D:$D(DG)>9 F^DIE17,DE S DQ=8,DW=".11;1",DV="Fa",DU="",DLB="STREET ADDRESS [LINE 1]",DIFLD=.111
 S DE(DW)="C8^DVBHCE17",DE(DW,"INDEX")=1
 G RE
...
C8S S X="" G:DG(DQ)=X C8F1 K DB
 D ^DVBHCE18
C8F1 N X,X1,X2 S DIXR=230 D C8X1(U) K X2 M X2=X D C8X1("O") K X1 M X1=X
 D
...
X9 K:X[""""!($A(X)=45) X I $D(X) K:$L(X)>30!($L(X)<3) X D:$D(X) UP^DGHELP
 ...
 Q
 ;
10 D:$D(DG)>9 F^DIE17,DE S DQ=10,DW=".11;3",DV="Fa",DU="",DLB="STREET ADDRESS [LINE 3]",DIFLD=.113
 S DE(DW)="C10^DVBHCE17",DE(DW,"INDEX")=1
 G RE
C10 G C10S:$D(DE(10))[0 K DB
 D ^DVBHCE21
C10S S X="" G:DG(DQ)=X C10F1 K DB
 D ^DVBHCE22
C10F1 N X,X1,X2 S DIXR=233 D C10X1(U) K X2 M X2=X D C10X1("O") K X1 M X1=X
 D
...
C10X1(DION) K X
 ...
 Q
C10F2 Q
X10 K:X[""""!($A(X)=45) X I $D(X) K:$L(X)>30!($L(X)<3) X
 I $D(X),X'?.ANP K X
 Q
 ;
11 D:$D(DG)>9 F^DIE17 G ^DVBHCE23
