...
DE S DIE="^DGCR(399,D0,""RC"",",DIC=DIE,DP=399.042,DL=2,DIEL=1,DU="" K DG,DE,DB Q:$O(^DGCR(399,D0,"RC",DA,""))=""
 I $D(^(0)) S %Z=^(0) S %=$P(%Z,U,13) S:%]"" DE(7)=% S %=$P(%Z,U,15) S:%]"" DE(1)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
X6 I $$FT^IBCEF(DA(1))'=3 S Y="@7591"
 Q
7 D:$D(DG)>9 F^DIE17,DE S DQ=7,DW="0;13",DV="F",DU="",DLB="UB92 FORM LOCATOR 49",DIFLD=.13
 G RE
X7 K:$L(X)>4!($L(X)<1) X
 I $D(X),X'?.ANP K X
 Q
