...
DE S DIE="^DGCR(399,",DIC=DIE,DP=399,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^DGCR(399,DA,""))=""
 ...
 I $D(^("M")) S %Z=^("M") S %=$P(%Z,U,4) S:%]"" DE(18)=% S %=$P(%Z,U,5) S:%]"" DE(19)=% S %=$P(%Z,U,11) S:%]"" DE(8)=% S %=$P(%Z,U,13) S:%]"" DE(1)=% S %=$P(%Z,U,14) S:%]"" DE(3)=%
 I $D(^("M1")) S %Z=^("M1") S %=$P(%Z,U,2) S:%]"" DE(13)=% S %=$P(%Z,U,3) S:%]"" DE(14)=% S %=$P(%Z,U,4) S:%]"" DE(15)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
X12 S:IBDR20'["32" Y="@33"
 Q
13 D:$D(DG)>9 F^DIE17,DE S DQ=13,DW="M1;2",DV="F",DU="",DLB="PRIMARY PROVIDER #",DIFLD=122
 G RE
X13 K:$L(X)>13!($L(X)<3) X
 I $D(X),X'?.ANP K X
 Q
 ;
14 S DW="M1;3",DV="F",DU="",DLB="SECONDARY PROVIDER #",DIFLD=123
 G RE
X14 K:$L(X)>13!($L(X)<3) X
 I $D(X),X'?.ANP K X
 Q
 ;
15 S DW="M1;4",DV="F",DU="",DLB="TERTIARY PROVIDER #",DIFLD=124
 G RE
...
X17 S:IBDR20'["33" Y="@34"
 Q
18 S DW="M;4",DV="F",DU="",DLB="MAILING ADDRESS NAME",DIFLD=104
 G RE
X18 K:$L(X)>30!($L(X)<1) X
 I $D(X),X'?.ANP K X
 Q
 ;
19 S DW="M;5",DV="FX",DU="",DLB="MAILING ADDRESS STREET",DIFLD=105
 G RE
X19 K:$L(X)>35!($L(X)<3) X
 I $D(X),X'?.ANP K X
 Q
 ;
20 D:$D(DG)>9 F^DIE17 G ^IBXSC35
