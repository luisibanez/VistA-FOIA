IBXSC7 ; GENERATED FROM 'IB SCREEN7' INPUT TEMPLATE(#513), FILE 399;10/15/04
 D DE G BEGIN
DE S DIE="^DGCR(399,",DIC=DIE,DP=399,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^DGCR(399,DA,""))=""
 I $D(^(0)) S %Z=^(0) S %=$P(%Z,U,22) S:%]"" DE(21)=% S %=$P(%Z,U,24) S:%]"" DE(11)=% S %=$P(%Z,U,25) S:%]"" DE(13)=% S %=$P(%Z,U,26) S:%]"" DE(17)=% S %=$P(%Z,U,27) S:%]"" DE(22)=%
...
X2 S:$D(IBOUT) Y="@999"
 Q
3 S D=0 K DE(1) ;43
 S DIFLD=43,DGO="^IBXSC71",DC="1^399.043DA^OP^",DV="399.043MDX",DW="0;1",DOW="OP VISITS DATE(S)",DLB="Select "_DOW S:D DC=DC_D
...
C13F1 Q
X13 S DIC("S")="I $P(^(0),U,23),$$TOBIN^IBCU4(Y,D0)" D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X
 Q
 ;
...
18 D:$D(DG)>9 F^DIE17,DE S DQ=18,DW="U2;2",DV="NJ3,0",DU="",DLB="COVERED DAYS",DIFLD=216
 G RE
X18 K:+X'=X!(X>999)!(X<0)!(X?.E1"."1N.N) X
 Q
 ;
...
C21F1 Q
X21 Q
22 D:$D(DG)>9 F^DIE17,DE S DQ=22,DW="0;27",DV="S",DU="",DLB="BILL CHARGE TYPE",DIFLD=.27
 S DE(DW)="C22^IBXSC7"
