IBXSC6 ; GENERATED FROM 'IB SCREEN6' INPUT TEMPLATE(#512), FILE 399;10/15/04
 D DE G BEGIN
DE S DIE="^DGCR(399,",DIC=DIE,DP=399,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^DGCR(399,DA,""))=""
 I $D(^(0)) S %Z=^(0) S %=$P(%Z,U,19) S:%]"" DE(23)=% S %=$P(%Z,U,22) S:%]"" DE(17)=% S %=$P(%Z,U,24) S:%]"" DE(5)=% S %=$P(%Z,U,25) S:%]"" DE(7)=% S %=$P(%Z,U,26) S:%]"" DE(11)=% S %=$P(%Z,U,27) S:%]"" DE(18)=%
...
C7F1 Q
X7 S DIC("S")="I $P(^(0),U,23),$$TOBIN^IBCU4(Y,D0)" D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X
 Q
 ;
...
12 D:$D(DG)>9 F^DIE17,DE S DQ=12,DW="U2;2",DV="NJ3,0",DU="",DLB="COVERED DAYS",DIFLD=216
 G RE
X12 K:+X'=X!(X>999)!(X<0)!(X?.E1"."1N.N) X
 Q
 ;
...
15 S DW="U2;7",DV="NJ3,0",DU="",DLB="CO-INSURANCE DAYS",DIFLD=221
 G RE
X15 K:+X'=X!(X>999)!(X<0)!(X?.E1"."1N.N) X
 Q
 ;
16 S DQ=17 ;@6121
...
C17F1 Q
X17 Q
18 D:$D(DG)>9 F^DIE17,DE S DQ=18,DW="0;27",DV="S",DU="",DLB="BILL CHARGE TYPE",DIFLD=.27
 S DE(DW)="C18^IBXSC6"
...
X20 S DIPA("FT1")=$P($G(^DGCR(399,DA,0)),U,19)
 Q
21 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=21 D X21 D:$D(DIEFIRE)#2 FIREREC^DIE17 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X21 D CKFT^IBCIUT1(IBIFN)
 Q
22 S DQ=23 ;@614
23 D:$D(DG)>9 F^DIE17,DE S DQ=23,DW="0;19",DV="RP353'",DU="",DLB="FORM TYPE",DIFLD=.19
 S DE(DW)="C23^IBXSC6"
 S DU="IBE(353,"
 G RE
