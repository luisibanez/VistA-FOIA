...
CHECK ; check if transfer pricing and not already added
 ...
 I $P(^RMPR(660,IBDA,"AM"),"^",3)'=1,$P(^("AM"),"^",3)'=4,'$D(^IBAT(351.67,"B",$P(IBDATA,"^",6))) Q
 ;
 Q:'$P(IBDATA,"^",16)  ; no total cost, at least yet
 ;
FILE ; ok transaction needs to be filled in tp files
 ;
 S IBDATA=$$RMPR^IBATFILE(IBDFN,IBDT,$$PPF^IBATUTL(IBDFN),(IBDA_";RMPR(660,"),$P(IBDATA,"^",6),$P(IBDATA,"^",16))
 ;
 Q
