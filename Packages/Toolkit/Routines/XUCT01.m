XUCT01 ; GENERATED FROM 'XUSERINQ' PRINT TEMPLATE (#27) ; 06/28/03 ; (FILE 200, MARGIN=80)
 G BEGIN
N W !
...
BEGIN ;
 ...
 W ?11 W:$L($P(^VA(200,D0,0),U,11)) !,$C(7),?3,"Terminated: ",DIPA("TD") K DIP K:DN Y
 ...
 D N:$X>0 Q:'DN  W ?0 W " "
 X DXS(18,9.2) S X1=DIP(2) S:X]""&(X?.ANP)&(X1'[U)&(X1'["$C(94)") DIPA($E(X,1,30))=X1 S X="" K DIP K:DN Y W X
 D N:$X>0 Q:'DN  W ?0 X DXS(19,9.2) S X=$S(DIP(1):DIP(2),DIP(3):X) K DIP K:DN Y W X
 D N:$X>0 Q:'DN  W ?0 X DXS(20,9) K DIP K:DN Y W X
 D N:$X>0 Q:'DN  W ?0 X DXS(21,9) K DIP K:DN Y W X
 D N:$X>0 Q:'DN  W ?0 X DXS(22,9) K DIP K:DN Y W X
 D N:$X>0 Q:'DN  W ?0 X DXS(23,9.2) S X=X_$P(DIP(2),U,3),DIP(3)=$G(X) S X=1,DIP(4)=$G(X) S X="",X=$S(DIP(1):DIP(3),DIP(4):X) K DIP K:DN Y W X
 D N:$X>0 Q:'DN  W ?0 X DXS(24,9.2) S X=$S(DIP(1):DIP(2),DIP(3):X) K DIP K:DN Y W X
...
B1R ;
 ...
 D T Q:'DN  D N W ?0 W ""
 W ?2 S Y=IOSL N IOSL S IOSL=$S(IOST["P-":999,1:Y),Y=D0 D EN^XMA7 K DIP K:DN Y
 D T Q:'DN  D N W ?0 W ""
 K Y
 Q
HEAD ;
