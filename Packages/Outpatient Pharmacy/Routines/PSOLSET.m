PSOLSET	;BHAM ISC/SAB - site parameter set up ;1:30 PM  3 Jan 2010
	...
	;W !,"Outpatient Pharmacy software - Version "_$P($T(V),";",3)
...
PLBL	I $P(PSOPAR,"^",8) D
	...
	S PSOAFIN=$P($G(^PS(59,PSOSITE,"RXFIN")),"^",1) ;vfah
	S PSOAFPFX=$S(PSOAFIN="Y":"Select LABEL PRINTER or FAX DEVICE: ",1:"Select LABEL PRINT: ") ;vfah
LBL	S %ZIS="MNQ",%ZIS("A")=PSOAFPFX S:$G(PSOCLBL)&($D(PSOLAP))!($G(SUSPT)) %ZIS("B")=$S($G(SUSPT):PSLION,1:PSOLAP) ;vfah
...
LASK	I $G(PSOPIOST),$D(^%ZIS(2,PSOPIOST,55,"B","LL")) G EXIT
	;
	;vfah AutoFinish fax additions begin here
	K PSOAFFXP,PSOAFFXL
	I PSOLAP["FAX" D
	.S PSOAFFXP="",PSOAFFXL="",PSOAFFXR=""
	.S PSOLAP="AFFAX" D
	..S PSOION="" S PSOION=$O(^%ZIS(1,"B",PSOLAP,PSOION))
	..I $D(^DIZ(22900)) D
	...S DIC="^DIZ(22900,",DIC(0)="AEQMZ",DIC("A")="SEND FAX TO: "
	...D ^DIC K DIC
	...I Y=-1 W !,"Invalid selection" G LBL
	...S PSOAFFXL=$P(Y,"^",2)
	...S PSOAFFXP=$P($G(^DIZ(22900,+Y,3)),"^",3)
	...S PSOAFFXR=PSOAFFXP
	...I PSOAFFXL=""!(PSOAFFXP="") G LBL
	I $G(PSOAFFXP)&(PSOLAP="AFFAX")'="" G EXIT
	;vfah Autofinish fax additions end here
	;
