PSOREF	;BIR/SAB-refill data entry ;12:03 PM  31 Dec 2008
	;;7.0;OUTPATIENT PHARMACY;**1,23,27,36,46,78,130,131,148,206,208**;DEC 1997;Build 54;WorldVistA 30-Jan-08
	;
...
OERR	;single refil
	...
	D ^DIC K DIC ;vfah
	S PSOZAF=+Y ;vfah
	I $P($G(^PSRX(RXN,"OR1")),"^",5)=$G(PSOZAF) S VALMBCK="",VALMSG="Refill option is not available for Autofinshed Rxs" K PSOZAF Q  ;vfah
	;WVEHR ;end p208
...
SPEED	;speed refill
	...
	..D ^DIC K DIC ;vfah
	..S PSOZAF=+Y ;vfah
	..I $P($G(^PSRX($P(PSOLST(ORN),"^",2),"OR1")),"^",5)=$G(PSOZAF) W $C(7),!!,"Rx "_$$GET1^DIQ(52,$P(PSOLST(ORN),"^",2),.01)_" is an Autofinish,Rx and can not be refilled" K PSOZAF D PAUSE^VALM1 Q  ;vfah
	..;WVEHR ;end p208
