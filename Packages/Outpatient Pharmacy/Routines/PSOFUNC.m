PSOFUNC ;BHAM ISC/DRI - functions moved from the psf global ; 10/26/92 11:49
 ;;7.0;OUTPATIENT PHARMACY;**146,223**;DEC 1997
...
INP5261 D EN^DDIOL("Duplicate Drug in Rx #"_$P(^PSRX(RX,0),"^")_" . Discontinue? (Y/N): ","","$C(7),?10") R ZX:DTIME
 ...
 S ZX=ZX?1"Y".E I ZX S $P(^PSRX(RX,"STA"),"^")=12,$P(^PSRX(RX,3),"^",5)=$S($P($G(^PSRX(RX,3)),"^"):$P(^(3),"^"),1:DT) D CAN^PSOTPCAN(RX) D EN^DDIOL("     Discontinued") Q
 K X,XS,ZS Q
