...
ENTRY ;Get Measurement Summary for a patient given a date range and a Provider
 ...
 N DATA,VDATE,VIEN,IEN,RETVAL,CNT,PT1,PT2
 ...
 S FIRST=ENDDT-1 F  S FIRST=$O(^AUPNVSIT("AA",DFN,FIRST)) Q:FIRST=""!(FIRST>BEGDT)!(+VIEN)  D
 .S IEN=0 F  S IEN=$O(^AUPNVSIT("AA",DFN,FIRST,IEN)) Q:'+IEN!(+VIEN)  D
 ..;Check provider, Only visits for chosen provider
 ..Q:'$$PRV^BGPMUUT1(IEN,BGPPROV)
 ..;Quit if the patient has a pregnancy diagnosis
 ..I ($P(^DPT(DFN,0),U,2)="F")&($$LASTDX^BGPMUUT2(DFN,BEGDT,ENDDT,"BGPMU PREGNANCY ALL ICD")) Q
 ..;Quit if the visit does not have a valid E&M code
 ..Q:'$$VSTCPT^BGPMUUT1(DFN,IEN,"BGPMU BMI ENC PEDS EM")
 ...
 .I BGPAGEE'>10 D NUMATOR(DFN,"BGPMU 2-10",.NUMCB1,.NUMCB2,.NUMCB3,.DNOMB,.NOTCB1,VSTSDT,VSTEDT,DEN)
 .;Range C (Ages 11-16 inclusive)
 .I BGPAGEE'<11 D NUMATOR(DFN,"BGPMU 11-16",.NUMCC1,.NUMCC2,.NUMCC3,.DNOMC,.NOTCC1,VSTSDT,VSTEDT,DEN)
 .;
 ...
 .;Subtract from the Final Denomator those records that DO NOT meet numerator criteria for each category
 .S NOA=FINALD1-$P(FINA,U,5)
 .S NOB=FINALD2-$P(FINB,U,5)
 .S NOC=FINALD3-$P(FINC,U,5)
 .;Get the Performance Calculation by dividing those that meet Numerator criteria divided by the final denominator
 .S PCA=$S(FINALD1>0:(NUMCA1+NUMCA2+NUMCA3)/FINALD1,1:"")
 .S PCB=$S(FINALD2>0:(NUMCB1+NUMCB2+NUMCB3)/FINALD2,1:"")
 .S PCC=$S(FINALD3>0:(NUMCC1+NUMCC2+NUMCC3)/FINALD3,1:"")
 .;Do performance calculations for the numbers NOT in the numerator
 .S NCA=$S(FINALD1>0:(FINALD1-(NUMCA1+NUMCA2+NUMCA3))/FINALD1,1:"")
 .S NCB=$S(FINALD2>0:(FINALD2-(NUMCB1+NUMCB2+NUMCB3))/FINALD2,1:"")
 .S NCC=$S(FINALD3>0:(FINALD3-(NUMCC1+NUMCC2+NUMCC3))/FINALD3,1:"")
 .;Store Final Date into work global for printing reports
 .S ^TMP("BGPMU 2-16",$J,BGPMUTF,"POPULATION")=FINA_U_PCA_U_NCA
 .S ^TMP("BGPMU 2-10",$J,BGPMUTF,"POPULATION")=FINB_U_PCB_U_NCB
 .S ^TMP("BGPMU 11-16",$J,BGPMUTF,"POPULATION")=FINC_U_PCC_U_NCC
 Q
 ;
NUMATOR(DFN,PATPOP,NUM1,NUM2,NUM3,DENOM,NONUM,VSTSDT,VSTEDT,DEN) ;Calculate Numerators
 ...
 I +J S NUM2=1 ;COUNSELING ON NUTRITION COUNTER
 I +K S NUM3=1 ;COUNSELING ON PHYSICAL ACTIVITY COUNTER
