BGP1DPP ; IHS/CMI/LAB - IHS GPRA 10 SELECTED REPORT DRIVER ALL PATIENTS 29 Apr 2011 8:36 PM ;
 ;;11.1;IHS CLINICAL REPORTING SYSTEM;;JUN 27, 2011;Build 33
 ;
 ;
 W:$D(IOF) @IOF
 W !!,$$CTR("2011 Clinical Performance Measure Report (Selected Measures)",80)
...
SETIND ;
 ...
 S DIR(0)=DIR(0)_";IPC:Improving Patient Care Measures;PQA:Pharmacy Quality Alliance Measures;SEL:Selected Performance Measures (User Defined)"
 S DIR("A")="Which set of Measures should be included in this report" KILL DA D ^DIR KILL DIR
 I $D(DIRUT) D XIT Q
 S BGPINDB=$E(Y)
 I BGPINDB="S" D SI I '$D(BGPIND) G SETIND
...
SUM ;display summary of this report
 W:$D(IOF) @IOF
 W !,$$CTR("SUMMARY OF 2011 CLINICAL MEASURE PERFORMANCE REPORT TO BE GENERATED")
 W !!,"The date ranges for this report are:"
 W !?5,"Report Period: ",?31,$$FMTE^XLFDT(BGPBD)," to ",?31,$$FMTE^XLFDT(BGPED)
 W !?5,"Previous Year Period: ",?31,$$FMTE^XLFDT(BGPPBD)," to ",?31,$$FMTE^XLFDT(BGPPED)
 W !?5,"Baseline Period: ",?31,$$FMTE^XLFDT(BGPBBD)," to ",?31,$$FMTE^XLFDT(BGPBED)
 ...
 W !!,"These measures will be calculated: " S X=0 F  S X=$O(BGPIND(X)) Q:X'=+X  W $P(^BGPINDB(X,0),U,3)," ; "
 W !!,"Lists will be produced for these measures: "
 S X=0 F  S X=$O(BGPLIST(X)) Q:X'=+X  W $P(^BGPINDB(X,0),U,3)," ; "
 D PT^BGP1DSL
 I BGPROT="" G LISTS
ZIS ;call to XBDBQUE
 D REPORT^BGP1UTL
...
EI ;
 ...
 Q
II ;
 S X=0 F  S X=$O(^BGPINDBC("AIPC",1,X)) Q:X'=+X  S BGPIND($P(^BGPINDBC(X,0),U,1))=""
 Q
PI ;
 S X=0 F  S X=$O(^BGPINDBC("APQA",1,X)) Q:X'=+X  S BGPIND($P(^BGPINDBC(X,0),U,1))=""
 Q
SI ;
 K BGPIND
 D EN^BGP1DSI
...
LISTS ;any lists with measures?
 ...
 I $D(BGPLIST) D RT^BGP1DSL I '$D(BGPLIST)!($D(BGPQUIT)) G LISTS ;get report type for each list
 Q
TSKMN ;EP ENTRY POINT FROM TASKMAN
 ...
 S ZTCPU=$G(IOCPU),ZTRTN="DRIVER^BGP1DPP",ZTDTH="",ZTDESC="GPRA 10 REPORT" D ^%ZTLOAD D XIT Q
 Q
 ;
NODEV ;
 S XBRP="",XBRC="NODEV1^BGP1DPP",XBRX="XIT^BGP1DPP",XBNS="BGP"
 D ^XBDBQUE
 ;D XIT
 Q
 ;
