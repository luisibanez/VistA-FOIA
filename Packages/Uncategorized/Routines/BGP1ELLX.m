BGP1ELLX ; IHS/CMI/LAB - print ind ;
 ...
 ;
I1AGE ;EP
 S BGPHD1="ACTIVE CLINICAL PATIENTS 65+",BGPHD2="Total Active Clinical 65+" S BGPHD1="ACTIVE CLINICAL PATIENTS =>65",BGPHD2="Active Clinical Patients 65+"
 S X=" " D S(X,1,1) D H3 S X=" " D S(X,1,1)
 K BGPDAC,BGPDAP,BGPDAB S C=0 F BGPX="AWD" F BGPX1=1,2,3 D I1AGE1
 D I1AGEP
MALE ;
 S BGPHD1="MALE ACTIVE CLINICAL PATIENTS 65+",BGPHD2="Male Active Clinical 65+"
 S X=" " D S(X,1,1) D H3 S X=" " D S(X,1,1)
 K BGPDAC,BGPDAP,BGPDAB S C=0 F BGPX="AWE" F BGPX1=1,2,3 D I1AGE1
 D I1AGEP
FEMALE ;
 S BGPHD1="FEMALE ACTIVE CLINICAL PATIENTS 65+",BGPHD2="Female Active Clinical 65+"
 S X=" " D S(X,1,1) D H3 S X=" " D S(X,1,1)
...
I1AGE1 ;
 S C=C+1
 S BGPF="ELD."_BGPX_"."_BGPX1 S BGPPC=$O(^BGPELIIB("C",BGPF,0))
...
I1AGEP ;
 ...
 S X=BGPHD2 D S(X,1,1) ;D S(BGPHD3,1,1)
 F X=1:1:3 S V=$P(BGPDAC(X),U) S Y=V D S(Y,,X+1)
 S X="# w/ Annual Wellness Exam" D S(X,1,1)
 F X=1:1:3 S V=$P(BGPDAC(X),U,2) S Y=V D S(Y,,X+1)
 S X="% w/ Annual Wellness Exam" D S(X,1,1)
 F X=1:1:3 S V=$P(BGPDAC(X),U,3) S Y=$$SB^BGP1ELL2($J(V,6,1)) D S(Y,,X+1)
PR ; 
 S X=" " D S(X,1,1) S X="PREVIOUS YEAR PERIOD" D S(X,1,1)
 S X=" " D S(X,1,1) S X=BGPHD2 D S(X,1,1) ;D S(BGPHD3,1,1)
 F X=1:1:3 S V=$P(BGPDAP(X),U) S Y=V D S(Y,,X+1)
 S X="# w/ Annual Wellness Exam" D S(X,1,1)
 F X=1:1:3 S V=$P(BGPDAP(X),U,2) S Y=V D S(Y,,X+1)
 S X="% w/ Annual Wellness Exam" D S(X,1,1)
 F X=1:1:3 S V=$P(BGPDAP(X),U,3) S Y=$$SB^BGP1ELL2($J(V,6,1)) D S(Y,,X+1)
 ;percentage changes
 S X=" " D S(X,1,1) S X="CHANGE FROM PREV YR %" D S(X,1,1)
 S X="w/ Annual Wellness Exam" D S(X,1,1)
 F X=1:1:3 S N=$P(BGPDAC(X),U,3),O=$P(BGPDAP(X),U,3) S Y=$$SB^BGP1ELL2($J((N-O),6,1)) D S(Y,,X+1)
BL ;
 ...
 S X="% w/ Annual Wellness Exam" D S(X,1,1)
 F X=1:1:3 S V=$P(BGPDAB(X),U,3) S Y=$$SB^BGP1ELL2($J(V,6,1)) D S(Y,,X+1)
 ;percentage changes
 S X=" " D S(X,1,1) S X="CHANGE FROM BASE YR %" D S(X,1,1)
 S X="w/ Annual Wellness Exam" D S(X,1,1)
 F X=1:1:3 S N=$P(BGPDAC(X),U,3),O=$P(BGPDAB(X),U,3) S Y=$$SB^BGP1ELL2($J((N-O),6,1)) D S(Y,,X+1)
 Q
...
CALC(N,O) ;
 ...
 Q Z
H3 ;EP
 S X="Annual Wellness Visit" D S(X,1,1) S Y=" " D S(Y,1,1) S X=BGPHD1 D S(X,1,1) S Y=" " D S(Y,1,1)
