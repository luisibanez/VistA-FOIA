...
RANGE(ABMFSCHD,ABMMLT,ABMDT) ;PEP - returns range of codes with charges
 ...
 .S ABMFDT=0,ABMSV=0,ABMFLG=0
 .F  S ABMFDT=$O(^ABMDFEE(ABMFSCHD,ABMMLT,ABMCIEN,1,"B",ABMFDT)) Q:(+$G(ABMFDT)=0)  D  Q:(ABMFLG'=0)
 ..I +$G(ABMSV)=0 S ABMSV=ABMFDT
 ..I (ABMDT>ABMSV&(ABMDT<ABMFDT))!(ABMDT=ABMSV) D
 ...S ABMDIEN=$O(^ABMDFEE(ABMFSCHD,ABMMLT,ABMCIEN,1,"B",ABMSV,0))
 ...S ABMFLG=1
 ...S ABMSV=$G(ABMFDT)
 .I ABMFLG=0,(+$G(ABMSV)'=""),((ABMDT>ABMSV)!(ABMDT=ABMSV)) S ABMDIEN=$O(^ABMDFEE(ABMFSCHD,ABMMLT,ABMCIEN,1,"B",ABMSV,0)),ABMFLG=1
 .I ABMFLG=1 D
