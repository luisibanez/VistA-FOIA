BLRUTIL ;IHS/ITSC/TPF - BLR LINK UTILITIES AND TESTING SUBROUTINES
 ;;5.2;IHS LABORATORY;**1013,1014,1015,1018,1019,1020,1024,1027**;NOV 01, 1997
 ;
...
CAPVARS(X,TARGET) ;EP
 K @X
 S Y="%" F %=0:0 S Y=$O(@Y) Q:Y=""  S %=$D(@Y) S:%#2&($E(Y,1,2)'="ZT") @(X_"("""_Y_""")=$G(@Y,""NULL"")")
 M @TARGET=@X
 Q
...
ENTRYAUD(LABEL,ARRY1,ARRY2,ARRY3) ;EP
 ...
 NEW %DT  ; <<-- 1019 MOD -- use local %DT
 S %DT="R" S X="N" D ^%DT S NOW=Y
 S ENTRYNUM=$G(^BLRENTRY)+1
 ;I '$D(^BLRENTRY) S ($P(^BLRENTRY,U,2),STARTTIM)=$H
 ;E  S STARTTIM=$P($P(^BLRENTRY,U,2),",",2)
 S NOWTIM=$P($H,",",2)
 ;I (NOWTIM-STARTTIM)>3600 S $P(^BLRENTRY,U,3)="SNAPSHOTS HAVE BEEN RUNNING FOR OVER AN HOUR- SNAPSHOTS STOPPPED!!" Q
 S $P(^BLRENTRY,U)=ENTRYNUM
 S ^BLRENTRY(DUZ,NOW,ENTRYNUM,LABEL)=""
 D CAPVARS("BLRVARS","^BLRENTRY(DUZ,NOW,ENTRYNUM,LABEL)")
 ; I $D(LRPARAM)>10 M ^BLRENTRY(DUZ,NOW,ENTRYNUM,LABEL,"LRPARAM")=LRPARAM
 ;
 ;I $G(ARRY)'="" D  ; Have an array that needs to be monitored; Merge it  
 ;. M ^BLRENTRY(DUZ,NOW,ENTRYNUM,LABEL,ARRY)=@ARRY
 ;----- BEGIN IHS/OIT/MKK -- 1024 MODIFICATION
...
FIXCH S U="^",COUNT=0
 ...
 ..I FLAG W !,"PROBLEM WITH UID ""C"" X-REF IN LRO(68" Q
 ..W !!,"^LR("_LRDFN_",""CH"","_LRIDT_",0)="_$G(^LR(LRDFN,"CH",LRIDT,0))
