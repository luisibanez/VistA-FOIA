BGP1DPA9 ; IHS/CMI/LAB - COMP NATIONAL GPRA FOR PTS W/APPT ;
...
ENDDATE ;
 ...
 ;ADDED FOR 12 REPORT
 S BGPBD=3110701,BGPED=3120630
 S BGPPBD=3100701,BGPPED=3110630
 S BGPPER=3120000
 S BGPNGR09=1
 S BGPLIST="A"
 S BGPCPLC=0
...
TSKMN ;EP ENTRY POINT FROM TASKMAN
 ...
 S ZTCPU=$G(IOCPU),ZTRTN="DRIVER^BGP1DPA9",ZTDTH="",ZTDESC="CRS 11 SCHED REPORT FOR 11" D ^%ZTLOAD D EOJ Q
 Q
EOJ ;
 D ^XBFMK
 K DIC,DIR,DFN
...
BDMG(BGPBD,BGPED,BGPCLN) ;EP - GUI DMS Entry Point
 ...
 D ^XBFMK
 K ZTSAVE S ZTSAVE("*")=""
 S ZTIO="",ZTDTH=$$NOW^XLFDT,ZTRTN="GUIEP^BGP1DPA9",ZTDESC="GUI CN GRPA APPT" D ^%ZTLOAD
 D EOJ
 Q
GUIEP ;EP
 D PROC
 K ^TMP($J,"BGP1DPA")
 S IOM=80
 D GUIR^XBLM("PRINT^BGP1DPA9","^TMP($J,""BGP1DPA"",")
...
GUIECP ;EP
 K ^TMP($J,"BGP1DPA")
 S IOM=80
 D GUIR^XBLM("CPPRINT^BGP1DPA9","^TMP($J,""BGP1DPA"",")
 S X=0,C=0 F  S X=$O(^TMP($J,"BGP1DPA",X)) Q:X'=+X  D
 .S BGPDATA=^TMP($J,"BGP1DPA",X)
 .I BGPDATA="ZZZZZZZ" S BGPDATA=$C(12)
 .S ^BGPGUIB(BGPIEN,11,X,0)=BGPDATA,C=C+1
...
PROC ;EP
 ...
 ..S X=$O(^DIBT("B","RPMS DEMO PATIENT NAMES",0)) I X Q:$D(^DIBT(X,1,Y))
