PXRMEFM ; SLC/PKR/PJH - Extract Finding Rule Management ;10/30/2002
 ;;2.0;CLINICAL REMINDERS;;Feb 04, 2005
 ;
 ;Main entry point for PXRM EXTRACT FINDINGS
START(PIEN) ;
 N PXRMDONE,VALMBCK,VALMCNT,VALMSG,X,XMZ,XQORM,XQORNOD
 S X="IORESET"
 D ENDR^%ZISS
 S VALMCNT=0
 D EN^VALM("PXRM EXTRACT FINDINGS")
 Q
 ;
BLDLIST ;Build workfile
 ...
 F IND=1:1:VALMCNT D
 .S ^TMP("PXRMEFM",$J,"IDX",IND,IND)=IEN(IND)
 Q
 ;
ENTRY ;Entry code
 D BLDLIST,XQORM
...
PEXIT ;PXRM EXCH MENU protocol exit code
 ...
 Q
 ;
XQORM S XQORM("#")=$O(^ORD(101,"B","PXRM EXTRACT FINDING SELECT ENTRY",0))_U_"1:"_VALMCNT
