PXRMOUTM ; SLC/PKR - MyHealtheVet output. ;12/30/2004
...
FOUT(INDENT,IFIEVAL,NLINES,TEXT) ;Do output for individual findings 
 ...
 ;
 ;================================================
MHVD(DEFARR,PXRMPDEM,PCLOGIC,RESLOGIC,RESDATE,FIEVAL) ;Prepare the 
 ...
 . ;D HEADER^PXRMOUT1(FTYPE,NFLINES,RESDATE,.NHDR,.HDR)
 .;Output the header and the finding text.
 . D COPYTXT^PXRMOUTU(.NTXT,NFLINES,.TEXT)
 D WEB(DEFARR("IEN"),.NTXT)
 Q
 ;
 ;================================================
MHVS(DEFARR,PXRMPDEM,PCLOGIC,RESLOGIC,RESDATE,FIEVAL) ;Prepare the 
 ;MyHealtheVet summary output.
 N NTXT
 S NTXT=0
 D LOGIC^PXRMFNFT(PXRMPDEM("DFN"),PCLOGIC,"PCL","S",.DEFARR,.FIEVAL,.NTXT)
 D LOGIC^PXRMFNFT(PXRMPDEM("DFN"),RESLOGIC,"RES","S",.DEFARR,.FIEVAL,.NTXT)
 D WEB(DEFARR("IEN"),.NTXT)
 Q
 ;
 ;================================================
WEB(RIEN,NTXT) ;Output the web site information.
 ...
 . S TEXT=$O(^PXD(811.9,RIEN,50,IEN,0))
 . S URL=$P(^PXD(811.9,RIEN,50,IEN,0),U,1)
 . S TITLE=$P(^PXD(811.9,RIEN,50,IEN,0),U,2)
