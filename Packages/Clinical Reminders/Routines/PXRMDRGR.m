PXRMDRGR ; SLC/PKR - Handle groups of drug findings. ;11/22/2004
...
FIEVAL(DFN,DRGRIEN,FINDPA,DEFARR,FINDING,XREF,FIEVAL) ;
 N DATE,DATEORDR,DRUGIEN,IND,FIEVT,FIEVTL,FINDL,NOCC,NFOUND,POI,RXTYL
 N SDIR,TDATE,TIND
 ...
 . K FIEVT
 . D DEVAL^PXRMDRUG(DFN,.FINDPA,.DEFARR,FINDING,DRUGIEN,POI,.FIEVT)
 . I FIEVT D
 .. S IND=0
 ...
 ;Order by date and eliminate duplicates; more than one drug can have
 ;the same Pharmacy Orderable Item.
 S DATE="",NFOUND=0
 F  S DATE=$O(DATEORDR(DATE),SDIR)  Q:(DATE="")!(NFOUND=NOCC)  D
 . S IND=0
 . F  S IND=$O(DATEORDR(DATE,IND)) Q:(IND="")!(NFOUND=NOCC)  D
 .. I '$D(FINDL(DATEORDR(DATE,IND))) D
 ... S NFOUND=NFOUND+1
 ... M FIEVAL(NFOUND)=FIEVTL(IND)
 ... S FINDL(DATEORDR(DATE,IND))=""
