...
DISP(VIEW) ;
 ...
 I $D(^TMP("PXRMEXTMP",$J,"DREPL",DDLG))>0 D DREPL^PXRMEXLC(DDLG,"")
 ;Process componentS
 D DCMP(DDLG,"","")
 ;Change header
...
DCMP(DLG,LEV,REPL) ;
 N DDATA,DDLG,DEND,DNAM,DSEQ,DSTRT,IND,JND,LAST,NUM
 S DSEQ=0,LAST=0
 I $G(REPL)="R" S REPL="(R)"
 ...
 .S NUM=$S($G(REPL)["R":DSEQ+1,1:DSEQ)
 .D DLINE(DNAM,LEV,NUM_$G(REPL)) Q:DREP'=""
 .;
 .;Process replacement elements
 .I $D(^TMP("PXRMEXTMP",$J,"DREPL",DNAM))>0 D DREPL^PXRMEXLC(DNAM,LEV)
 .;Process any sub-components
 .I $D(^TMP("PXRMEXTMP",$J,"DMAP",DNAM)) D DCMP(DNAM,LEV_DSEQ_".",$G(REPL))
 .;Extra line feed
 .I LEV="" D
 ..S NLINE=NLINE+1,^TMP("PXRMEXLD",$J,NLINE,0)=$J("",79)
 ..S ^TMP("PXRMEXLD",$J,"IDX",NLINE,NSEL)=""
