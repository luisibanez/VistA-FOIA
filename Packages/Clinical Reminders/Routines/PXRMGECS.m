PXRMGECS ;SLC/JVS GEC-Reports-cont'd ;6/19/03  20:58
...
DFN2 ;DFN array for By Provider Report
 ...
 .I FOR W:FLAG=1 !!,IOUON,$$GET1^DIQ(200,DOCT,.01)_" ("_DOCT_")",IOUOFF,?40,"Total # Complete Referrals: ",$G(^TMP("PXRMGEC",$J,"REFDOCC",$$GET1^DIQ(200,DOCT,.01),DOCT)) D PB Q:Y=0
 .I FOR D PB Q:Y=0
 .S DFN=0 F  S DFN=$O(@REF@("DFN",DOCT,DFN)) Q:DFN=""!(Y=0)  D
 ..S DIADA=0 F  S DIADA=$O(@REF@("DFN",DOCT,DFN,DIADA)) Q:DIADA=""  D
 ...S DATEV=0 F  S DATEV=$O(@REF@("DFN",DOCT,DFN,DIADA,DATEV)) Q:DATEV=""!(Y=0)  D
 ....I FOR W !,?2,$S(DFN'=DFN1!(FLAG=1):$P($G(^DPT(DFN,0)),"^",1)_" ("_$P($G(^DPT(DFN,0)),"^",9)_")",1:"") D PB Q:Y=0
 ....I FOR I DFN'=DFN1!(FLAG=1) W ! D PB Q:Y=0
 ....S FLAG=0
 ....W ?17,$P($P($G(^PXRMD(801.41,DIADA,0)),"^",1)," ",3,6),?41,$P($$FMTE^XLFDT(DATEV,"5ZM"),"@",1)
 ....S DFN1=DFN
 ....I FOR D PB Q:Y=0
 ....I 'FOR W !,$$GET1^DIQ(200,DOCT,.01)_"^"_DOCT,"^",$P($G(^DPT(DFN,0)),"^",1)_"^"_$P($G(^DPT(DFN,0)),"^",9),"^",$P($P($G(^PXRMD(801.41,DIADA,0)),"^",1)," ",3,6),"^",$P($$FMTE^XLFDT(DATEV,"5ZM"),"@",1)
 K ^TMP("PXRMGEC",$J)
 Q
 ;
...
CTP ;Referrals Counts by Patient
 ...
 W !,"From: "_$$FMTE^XLFDT(BDT,"5ZM")_" To: "_$$FMTE^XLFDT(EDT,"5ZM")
 W !,"Report Displays Counts of Complete Referrals Only"
 I FOR W !,"Patient",?25,"SSN",?37,"Total Count"
