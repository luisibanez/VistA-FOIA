LRVER3A ;SLC/CJS/DALOI/FHS - DATA VERIFICATION ;8/11/97 [ 04/29/2003  6:55 AM ]
 ;;5.2T9;LR;**1010,1013,1015,1018**;Nov 17, 2004
 ;;5.2;LAB SERVICE;**1,5,42,100,121,153,190,221,254,263,266,274**;Sep 27, 1994
...
XREF ;from COM1^LRVER4 and VER^LRVER3A
 ...
 ;
IHS ; 
 ;D:BLRLOG ^BLREVTQ("M","R",$G(BLROPT),,$S($D(LRACC):LRACC,1:^LRO(68,LRAA,1,LRAD,1,LRAN,.2)))
...
REQ ;
 ...
 ;----- BEGIN IHS MODIFICATION LR*5.2*1018
 I $D(LRSB(X)),LRSB(X)'["pending",LRSB(X)'="" Q  ;IHS/ITSC/TPF 10/2/01 FIX FOR SOME TESTS NOT SHOWING AS PENDING; REPORTED BY MITRETEK DURING E-SIG DEVELOPMENT THIS OCCURRED WHEN USING FAST BYPASS ;THIS ALONG WITH ADDITION IN CRSFLDS^BLRTN FIXES DUPLICATE ENTRIES IN LOG FILE
 ;----- END IHS MODIFICATION
 S D1=0 N A,LRPPURG
