LRMIVER1 ; IHS/DIR/FJE - MICRO CHART COPY APPROVAL CONT. 2/19/91 11:01 ; [ 11/19/2002  7:12 AM ]
...
APPROVE I '$O(^LRO(68,"AVS",LRAA,0)) W !,"No data." Q
 ...
 F I=0:0 W !!,"Do you wish to review the data as the (W)ards will see it, as the (L)ab",!,"will see it, or (N)ot review the data?  W// " R X:DTIME S:'$T X=U S:'$L(X) X="W" Q:X[U!("WLN"[X&($L(X)=1))  D INFO^LRMINEW  ;IHS/ANMC/CLS 08/18/96
 ...
 S:X="W" LRWRDVEW="" F I=0:0 W !,"Do you want to queue the data to print and approve it later" S %=2 D YN^DICN Q:%  W !,"Answer 'Y'es or 'N'o"  ;IHS/ANMC/CLS 08/18/96
...
S1 S LRDFN=+^LRO(68,"AVS",LRAA,LRAD,LRAN),LRIDT=$P(^(LRAN),U,2)
 ...
 I $D(^LR(LRDFN,"MI",LRIDT,0)) S LRVLOC=$S($L($P(^(0),U,8)):$P(^(0),U,8),1:0),^TMP($J,LRAN,LRDFN,LRIDT)=^(0)  ;IHS/ANMC/CLS 08/18/96 sort by LRAN
 ;S ^TMP($J,LRVLOC,LRDFN,LRIDT,1)=LRAD
 S ^TMP($J,LRAN,LRDFN,LRIDT,1)=LRAD  ;IHS/ANMC/CLS 08/18/96 LRAN
 Q
PRINT ;S LRVLOC="" F LRLCNT=0:0 S LRVLOC=$O(^TMP($J,LRVLOC)) Q:LRVLOC=""  S LRLTR=$E(LRVLOC,1,4) W @IOF D ^LRLTR:$E(IOST,1,2)'="C-",P1 Q:LREND
 S LRAN="" F LRLCNT=0:0 S LRAN=$O(^TMP($J,LRAN)) Q:LRAN=""  W @IOF D P1 Q:LREND  ;IHS/ANMC/CLS 08/18/96 LRAN
 Q
P1 ;S LRDFN=0 F I=0:0 S LRDFN=+$O(^TMP($J,LRVLOC,LRDFN)) Q:LRDFN<1  D P2 Q:LREND
 S LRDFN=0 F I=0:0 S LRDFN=+$O(^TMP($J,LRAN,LRDFN)) Q:LRDFN<1  D P2 Q:LREND  ;IHS/ANMC/CLS 08/18/96
 Q
P2 ;S LRIDT=0 F I=0:0 S LRIDT=+$O(^TMP($J,LRVLOC,LRDFN,LRIDT)) Q:LRIDT<1  D P3 Q:LREND
 S LRIDT=0 F I=0:0 S LRIDT=+$O(^TMP($J,LRAN,LRDFN,LRIDT)) Q:LRIDT<1  D P3 Q:LREND  ;IHS/ANMC/CLS 08/18/96 LRAN
 Q
P3 ;S LRWLSAVE=LRAA,LRLLT=^TMP($J,LRVLOC,LRDFN,LRIDT),LRACC=$P(LRLLT,U,6),LRAD=$E(LRLLT)_$P(LRACC," ",2)_"0000",X=$P(LRACC," "),DIC=68,DIC(0)="M"
 S LRWLSAVE=LRAA,LRLLT=^TMP($J,LRAN,LRDFN,LRIDT),LRACC=$P(LRLLT,U,6),LRAD=$E(LRLLT)_$P(LRACC," ",2)_"0000",X=$P(LRACC," "),DIC=68,DIC(0)="M"  ;IHS/ANMC/CLS 08/18/96
