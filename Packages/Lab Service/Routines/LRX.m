LRX ;SLC/BA/DALISC/FHS - UTILITY ROUTINES -- PREVIOUSLY ^LAB("X","...") ;DEC 09, 2008 8:30 AM
 ;;5.2;LAB SERVICE;**1004,1006,1018,1025**;NOV 01, 1997
 ;;5.2;LAB SERVICE;**65,153**;Sep 27, 1994
PT ;patient info
 ;
 N X,I,N,Y
 ;D KVAR^VADPT
 ...
 ;D KVAR^BLRDPT  ;IHS/DIR TUC/AAB 06/25/98
 D @$S($$ISPIMS^BLRUTIL:"KVAR^VADPT",1:"KVAR^BLRDPT")
 ;----- END IHS MODIFICATIONS
 K LRTREA,LRWRD S (AGE,PNM,SEX,DOB,SSN,HRCN,VA200,LRWRD,LRRB,LRTREA,VA("PID"),VA("BID"))=""  ;IHS/ANMC/CLS 08/18/96 subrtn returns HRCN & IHS printable age
 ...
 .;----- BEGIN IHS MODIFICATIONS LR*5.2*1018
 .;D OERR^BLRDPT D:'VAERR  ;IHS/DIR TUC/AAB 06/25/98
 .D @$S($$ISPIMS^BLRUTIL:"OERR^VADPT",1:"OERR^BLRDPT") D:'VAERR
 . . ;S PNM=VADM(1),SEX=$P(VADM(5),U),DOB=$P(VADM(3),U),AGE=VADM(4),SSN=$P(VADM(2),U),LRWRD=$P(VAIN(4),U,2)
 .. S PNM=VADM(1),SEX=$P(VADM(5),U),DOB=$P(VADM(3),U,2),AGE=VADM(4),SSN=$P(VADM(2),U),LRWRD=$P(VAIN(4),U,2) ;IHS/ANMC/CLS 09/28/96
 ..;----- END IHS MODIFICATIONS
...
DEM ;Call DEM^VADPT instead of OERR used above
 ...
 . ;D DEM^VADPT D:'VAERR
 .;----- BEGIN IHS MODIFICATIONS LR*5.2*1018
 . ;D DEM^BLRDPT D:'VAERR  ;IHS/DIR TUC/AAB 06/25/98
 . D @$S($$ISPIMS^BLRUTIL:"DEM^VADPT",1:"DEM^BLRDPT") D:'VAERR
 ..;----- END IHS MODIFICATIONS
...
LRUID(LRAA,LRAD,LRAN) ;Extrinsic function call to create a unique 
 ...
 ; L +^LRO(68,"C")
 ; I $D(^LRO(68,"C",LRUID)) D
 ; ----- BEGIN IHS/OIT/MKK -- LR*5.2*1025 MODIFICATIONS
 ;       Modifying Code per VA's LR*5.2*217 Patch
 L +^LRO(68,"C"):99999
 I $D(^LRO(68,"C",LRUID)),'$D(^LRO(68,"C",LRUID,LRAA,LRAD,LRAN)) D
 . ; ----- END IHS/OIT/MKK -- LR*5.2*1025 MODIFICATIONS
 . N X
 . S X=$E(LRUID,3,10)
 . ; F  S LRUID="00"_X L +^LRO(68,"C",LRUID) Q:'$D(^LRO(68,"C",LRUID))  L -^LRO(68,"C",LRUID) S X=X+1 S:X>99999999 X=11111111
 . ; ----- BEGIN IHS/OIT/MKK -- LR*5.2*1025 MODIFICATIONS
 . ;       Modifying Code per VA's LR*5.2*217 Patch
...
KVAR ;Kill laboratory/VADPT patient demographics
 ...
 ;D KVA^VADPT
 ;----- BEGIN IHS MODIFICATIONS LR*5.2*1018
 K LRTREA,LRWRD,PNM,SEX,DOB,SSN,HRCN,LRWRD,LRRB,LRTREA,VA,LRDFN,LRDPF,LREND,VAERR  ;IHS/ANMC/CLS 08/18/96
 ;D KVA^BLRDPT  ;IHS/DIR TUC/AAB 06/25/98
 D @$S($$ISPIMS^BLRUTIL:"KVA^VADPT",1:"KVA^BLRDPT")
 ;----- END IHS MODIFICATIONS
 Q
 ;----- BEGIN IHS MODIFICATIONS LR*5.2*1018 REPLACE VADPT CALLS WITH BLRDPT CALLS IF PIMS
ADDPT ;Returns VAPA( Patient data
 ;N X,I,N,Y D ADD^VADPT Q
 N X,I,N,Y D @$S($$ISPIMS^BLRUTIL:"ADD^VADPT",1:"ADD^BLRDPT") Q
OPDPT ;Returns VAPD( Patient data
 ;N X,I,N,Y D OPD^VADPT Q
 N X,I,N,Y D @$S($$ISPIMS^BLRUTIL:"OPD^VADPT",1:"OPD^BLRDPT") Q
SVCPT ;Returns VASV( Patient data
 ;N X,I,N,Y D SVC^VADPT Q
 N X,I,N,Y D @$S($$ISPIMS^BLRUTIL:"SVC^VADPT",1:"SVC^BLRDPT") Q
OADPT ;Returns VAOA( Patient data
 ;N X,I,N,Y D OAD^VADPT Q
 N X,I,N,Y D @$S($$ISPIMS^BLRUTIL:"OAD^VADPT",1:"OAD^BLRDPT") Q
INPPT ;Returns VAIN( Patient data
 ;N X,I,N,Y D INP^VADPT Q
 N X,I,N,Y D @$S($$ISPIMS^BLRUTIL:"INP^VADPT",1:"INP^BLRDPT") Q
IN5PT ;Returns VAIP( Patient data
 ;N X,I,N,Y D IN5^VADPT Q
 N X,I,N,Y D @$S($$ISPIMS^BLRUTIL:"IN5^VADPT",1:"IN5^BLRDPT") Q
PIDPT ;Returns VA("PID") and VA("BID") Patient Identifier
 ;N X,I,N,Y D PID^VADPT Q
 N X,I,N,Y D @$S($$ISPIMS^BLRUTIL:"PID^VADPT",1:"PID^BLRDPT") Q
 ;----- BEGIN IHS MODIFICATIONS LR*5.2*1025
 ; Failsafe Quit
 Q
 ; ----- END IHS MODIFICATIONS LR*5.2*1025
 ;----- END IHS MODIFICATIONS LR*5.2*1018 REPLACE VADPT CALLS WITH BLRDPT CALLS IF PIMS
 ;
 ;----- BEGIN IHS MODIFICATIONS LR*5.2*1018
 ;--- THE FOLLOWING SUBROUTINE WAS IN THE LRX ROUTINE BEFORE VA
 ;--- PATCHES WERE INSTALLED. IT IS PLACED HERE BECUASE IT IS STILL
