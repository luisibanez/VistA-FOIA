LRMIV1 ; IHS/DIR/FJE - LAB ROUTINE DATA VERIFICATION 2/6/91 08:21 ; [ 11/19/2002  7:11 AM ]
...
T4 Q:LRSQ'>0  K LRPRGSQ(LRSQ)
 ...
 S LRDPF=$P(^LR(LRDFN,0),U,2),DFN=$P(^(0),U,3),LRUNDO=0 D PT^LRX W ?25,"  ",PNM,?47," ",HRCN  ;IHS/ANMC/CLS 08/18/96
...
T6 R !,"ENTER 'E' TO EDIT OR INITIALS TO VERIFY: ",X:DTIME
 I X="E" D PAT1^LRMIV2 K LRPRGSQ W !,"DATA APPROVED BUT NOT VERIFIED",! G T51
 ;I $L(X)>1,$O(^VA(200,"C",X,0))=DUZ S $P(^LR(LRDFN,"MI",LRIDT,0),U,3)=DT,^(1)=DT_"^F^"_DUZ W !,"DATA APPROVED AND VERIFIED",! Q
 ;
 ;Above line modified per Appendix A of RPMS Lab E-Sig Enhancement v 5.2 Techincal Manual IHS/HQW/SCR - 8/23/01 
 I $L(X)>1,$O(^VA(200,"C",X,0))=DUZ D  Q  ;IHS/HQW/SCR - 8/23/01 
 .S $P(^LR(LRDFN,"MI",LRIDT,0),U,3)=DT,^(1)=DT_"^F^"_DUZ ;IHS/HQW/SCR - 8/23/01
 .W !,"DATA APPROVED AND VERIFIED",! ;IHS/HQW/SCR - 8/23/01 
 .I $$ADDON^BLRUTIL("LR*5.2*1013","BLRALAF",DUZ(2)) D ^BLRALAF ;IHS/HQW/SCR - 8/23/01
 .Q  ;IHS/HQW/SCR - 8/23/01
 ;
