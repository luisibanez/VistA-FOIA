...
QUE U IO K ^TMP($J),^TMP("LRDFN",$J) S LRQ(1)=^DD("SITE"),(LRQ,LR("Q"))=0 D L^LRU,H S LR("F")=1
 F B=LRS:0 S B=$O(^LRO(69,B)) Q:'B!(B>LRE)  F I=0:0 S I=$O(^LRO(69,B,1,I)) Q:'I  S X=+^(I,0),^TMP("LRDFN",$J,X)=""
...
SET S X=^LR(LRDFN,"CH",LRI,0),Y=$P(X,"^",6) S:Y="" Y="?? ?? ??" S ^TMP($J,$P(Y," "),$P(Y," ",2,3),+X,LRDFN,LRI)=$P(X,"^",5) Q
 ...
 ;S X=^LR(LRDFN,"CH",LRI,0),Y=$P(X,"^",6) S:Y="" Y="?? ?? ??" S ^TMP($J,$P(Y," "),"CHANGE",+X,LRDFN,LRI)=$P(X,"^",5) Q
