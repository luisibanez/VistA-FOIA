LRMIEDZ2 ;SLC/CJS/BA,AVAMC/REG - MICROBIOLOGY EDIT ROUTINE ; 2/22/89  11:16 ; [ 04/24/2003  12:50 PM ]
 ;;5.2T9;LR;**1010,1013,1015,1018**;Nov 17, 2004
 ;;5.2;LAB SERVICE;**23,104,242**;Sep 27, 1994
...
PAT1 S LRDFN=+^LRO(68,LRAA,1,LRAD,1,LRAN,0),LRIDT=9999999-^(3),LRCDT=+^(3),LREAL=$P(^(3),U,2),LRI=+$O(^(5,0)),LRSPEC=$S($D(^(LRI,0)):+^(0),1:"")
 ...
 S LRDPF=$P(^LR(LRDFN,0),U,2),DFN=$P(^(0),U,3),LRUNDO=0 D PT^LRX W !!,?5,PNM,"  HRCN: ",HRCN W:LRDPF=2 "  LOC: ",$S($L(LRWRD):LRWRD,1:$S($D(^LR(LRDFN,.1)):^(.1),1:"??"))  ;IHS/ANMC/CLS 08/18/96
 ...
 ;I LRTX(LRI)'["S DR=""[",LRSB X LRTX(LRI) D:'LREND EC3 K DR D:LRUNDO&$P($G(^LR(LRDFN,"MI",LRIDT,LRSB)),U)'="" VT^LRMIUT1 Q
 ;-----BEGIN IHS MODIFICATION LR*5.2*1018
 ;Above line modified per Appendix A of RPMS Lab E-sig Enhancement Version 5.2 tech manual IHS/HQW/SCR - 8/23/01 
 I LRTX(LRI)'["S DR=""[",LRSB X LRTX(LRI) D:'LREND EC3 D:BLRLOG ^BLREVTQ("M","R","MICRO") K DR D:LRUNDO&$P($G(^LR(LRDFN,"MI",LRIDT,LRSB)),U)'="" VT^LRMIUT1 D TRIG Q  ;IHS/HQW/SCR - 8/23/01
 ...
 S DR=DR(1,63.05) D ^DIE,EC3 K DR
 Q
...
ORDER S $P(^LRO(69,LRSSCOD,1,J,0),U,3)=$P(LRSSCN,U,2)
 ...
 ;----- BEGIN IHS MODIFICATION LR*5.2*1018
TRIG ;
 ;TRIG tag and following code added per Appendix A RPMS E-Sig EnhancementV 5.2 Techinical Manual IHS/HQW/SCR - 8/23/01
