LREPIAK ;DALCIO/HOAK EXTRACT ACKNOWLEDGEMENT ; 4/10/98
...
TEST ;
 ...
 S LRMID=HLMTIEN D
 . S LRMN=$P($G(^HL(772,LRMID,"IN",1,0)),HLFS,10)
 . I '$G(LRMN) S LRMN=$P($G(HLHDR),HLFS,10)
 . Q:'$G(LRMN)
 . I $G(HLHDR) S LRNCHAAC=$P(HLHDR,HLFS,3)
 . I '$G(LRNCHAAC) S LRNCHAAC=$P($G(^HL(772,LRMID,"IN",1,0)),HLFS,3)
 . S LRSTATUS=+$G(^HL(772,LRMID,"P")) I 'LRSTATUS S LRSTATUS=3
...
MAIL I $G(DUZ)'>0 S LRDUZ2=.5
 ...
 .  S LRX=0
 .  F  S LRX=$O(^TMP($J,"LREPIACK",LRX)) Q:+LRX'>0  S LRSTATUS=$P(^(LRX),U),LRSTADT=$P(^(LRX),U,2) D
 ..  I $G(LRSTATUS) S LRTXT(LRX)="Message: "_LRX_" "_$P(^HL(771.6,+LRSTATUS,0),U)_" @ "_$$FMTE^XLFDT(LRSTADT)
