LRAPAUSR ;AVAMC/REG/WTY - AUTOPSY SUPPLEMENTARY REPORT ;9/25/00 [ 04/11/2003  9:45 AM ]
 ;;5.2T9;LR;**1003,1018**;Nov 17, 2004
 ;;5.2;LAB SERVICE;**1,173,248**;Sep 27, 1994
 ;
...
DEV S ZTRTN="QUE^LRAPAUSR" D BEG^LRUTL Q:POP!($D(ZTSK))
...
LST K:'$D(LRSAV) ^LRO(69.2,LRAA,3) K LRAURPT
 ...
 I $D(LR("F")),IOST?1"C".E,'LR("Q") D M^LRU Q:LR("Q")  W @IOF
 W:IOST'?1"C".E&($E(IOST,1,2)'="P-"!($D(LR("FORM")))) @IOF
 K LRSAV D K^LRU,END^LRUTL Q
W W !,LR("%") Q
...
EN S LRQ=0,X=^LR(LRDFN,0) Q:'$O(^LR(LRDFN,84,0))  D ^LRUP
 ...
 F LRA=0:0 S LRA=$O(^LR(LRDFN,84,LRA)) Q:'LRA  D
 .S LRB=^(LRA,0)
 ...
 Q:LR("Q")  D FT Q
WRT D E S LRC=0
 F LRZ=0:1 S LRC=$O(^LR(LRDFN,84,LRA,1,LRC)) Q:'LRC  D
...
SUPA ;Print supplementary report audit information
 ...
 S B=0 F A=0:0 S A=$O(^LR(LRDFN,84,LRA,2,A)) Q:'A!(LR("Q"))  D
 .S B=A
 Q:LR("Q")
 Q:'$D(^LR(LRDFN,84,LRA,2,B,0))  S A=^(0)
...
SUPM ;Print previous versions of supplementary reports
 ...
 ;
 F A=0:0 S A=$O(^LR(LRDFN,84,LRA,2,A)) Q:'A!(LR("Q"))  D
...
FT F X=1:1 Q:$Y>(IOSL-13)  W !
 ...
 Q
SGL S X="" D ^LRUPS G:LRAN="?" SGL Q:LRAN=-1
 I $D(LR("AU1")),'$P(^LR(LRDFN,"AU"),U,15) D  Q
 .W $C(7),!!,"Report not verified."
 D SPC Q:X=""!(X[U)  S LRAP=LRDFN,LRSAV=1 D EN2^LRUA G DEV
END W $C(7),!!,"OK to delete the AUTOPSY SUPPLEMENTARY REPORT list "
 S %=2 D YN^LRU
 I %=1 K ^LRO(69.2,LRAA,3) S ^LRO(69.2,LRAA,3,0)="^69.29A^0^0" D  Q
 .W $C(7),!,"LIST DELETED !",!
 W !!,"OK, LET'S FORGET IT.",! Q
TOP S Z=$P(X,"|TOP|",1)_$P(X,"|TOP|",2) D FT,H S X=Z,LR("F")=1 Q
