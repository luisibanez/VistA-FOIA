LRMISTF1 ;SLC/CJS/BA/DALOI/FHS - MASS DATA ENTRY INTO FILE 63.05 ;8/11/97 [ 04/14/2003  10:12 AM ]
 ;;5.2T9;LR;**1010,1013,1015,1018**;Nov 17, 2004
 ;;5.2;LAB SERVICE;**121,128,202,263,264**;Sep 27, 1994
...
GET S X1="",LREND=0 I LRMODE<3 D
 ...
 F I=0:0 W !,"Verify all work automatically" S %=2 D YN^DICN Q:%  W !,"Answer 'Y'es or 'N'o"  ;IHS/ANMC/CLS 08/18/96
 ...
 ;----- BEGIN IHS MODIFICATIONS LR*5.2*1018
 S LRCO=0 F I=0:0 W !,"Designate the individual test as complete" S %=1 D YN^DICN Q:%  W !,"Answer 'Y'es or 'N'o"  ;IHS/ANMC/CLS 08/18/96
...
STUFF I '$D(^LRO(68,LRAA,1,LRAD,1,LRAN,0))!'$D(^(3)) W !,"Acc: ",LRAN," not set up." Q
 ...
 ;----- BEGIN IHS MODIFICATIONS LR*5.2*1018
 S LRDPF=$P(^LR(LRDFN,0),U,2),DFN=$P(^(0),U,3) D PT^LRX W ?15,PNM,?45,HRCN I LRDPF=2,$D(^DPT(DFN,.1)) W ?65,^(.1)  ;IHS/ANMC/CLS 08/18/96
 ;----- END IHS MODIFICATIONS
 I LRDPF=2,DFN,$P($G(^ORD(100.99,1,"CONV")),"^")=0 D EN^LR7OV2(DFN_";DPT(",1)
 W ! S LRCDT=+^LRO(68,LRAA,1,LRAD,1,LRAN,3),LRIDT=+$P(^(3),U,5),DIE="^LR("_LRDFN_",""MI"",",DA=LRIDT D ^DIE I $D(Y) S LREND=1 Q
 ;----- IHS BEGIN MODIFICATION LR*5.2*1018
 D:BLRLOG ^BLREVTQ("M","R","MICRO",,LRAA_","_LRAD_","_LRAN)
 ;----- IHS END MODIFICATION
 ...
 ;Following line added per Appendix A of RPMS E-Sig Enhancement V 5.2 technical manual IHS/HQW/SCR - 8/23/01 
 I $$ADDON^BLRUTIL("LR*5.2*1013","BLRALAF",DUZ(2)) D ^BLRALAF ;IHS/HQW/SCR - 8/23/01 
 D STF^LRMIUT  ;IHS/ANMC/CLS 08/18/96 not verified always appears on VT
