LA7VQIN1 ;VA/DALOI/JMC - Process Incoming UI Msgs, continued ;JUL 06, 2010 3:14 PM
...
NXTMSG ;
 ...
 I $D(^TMP("LA7 AMENDED RESULTS",$J)) D SENDARB^LA7VQN1A
 ;
 ; If cancelled orders received then send bulletins
 I $D(^TMP("LA7 ORDER STATUS",$J)) D SENDOSB^LA7VQN1A
 ;
 ; If units/normals changed then send bulletins
 I $D(^TMP("LA7 UNITS/NORMALS CHANGED",$J)) D SENDUNCB^LA7VQN1A
 ;
 ; If abnormal/critical results then send bulletins
 I $D(^TMP("LA7 ABNORMAL RESULTS",$J)) D SENDACB^LA7VQN1A
 ;
 D KILLMSH
 ;
 Q
...
NTE ;; Process NTE segment
 ...
 I LA7QUIT=2 Q
 ;
 I LA7MTYP="ORU" D NTE^LA7VQIN2
 I LA7MTYP="ORM" D NTE^LA7VQIN2
 I LA7MTYP="ORR" D NTE^LA7VQIN2
 ;
 Q
 ;
 ;
OBR ;; Process OBR segment
 ...
 . D CREATE^LA7LOG(LA7ERR)
 ;
 I LA7MTYP="ORR" D OBR^LA7VQIN4
 I LA7MTYP="ORU" D OBR^LA7VQIN4
