RACTQE ; GENERATED FROM 'RA QUICK EXAM ORDER' INPUT TEMPLATE(#1086), FILE 75.1;09/05/05
 D DE G BEGIN
DE S DIE="^RAO(75.1,",DIC=DIE,DP=75.1,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^RAO(75.1,DA,""))=""
 I $D(^(0)) S %Z=^(0) S %=$P(%Z,U,2) S:%]"" DE(2)=%,DE(9)=% S %=$P(%Z,U,4) S:%]"" DE(27)=% S %=$P(%Z,U,8) S:%]"" DE(14)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
X21 S:'$D(^RAMIS(71.2,"AB",+$$ITYPE^RASITE(RAPRI),RAMODPRO)) Y="@30"
 Q
22 S D=0 K DE(1) ;125
 S DIFLD=125,DGO="^RACTQE1",DC="1^75.1125PA^M^",DV="75.1125M*P71.2'X",DW="0;1",DOW="PROCEDURE MODIFIERS",DLB="Select "_DOW S:D DC=DC_D
 S DU="RAMIS(71.2,"
...
X26 I '$D(RACAT) S RACAT="I"
 Q
27 S DW="0;4",DV="SX",DU="",DLB="CATEGORY OF EXAM",DIFLD=4
...
X28 I '$D(RAPREOP1) S Y="@40"
 Q
29 D:$D(DG)>9 F^DIE17 G ^RACTQE2
