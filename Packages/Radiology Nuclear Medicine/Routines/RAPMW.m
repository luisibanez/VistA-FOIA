RAPMW ;HOIFO/SWM-Radiology Wait Time reports ;11/07/05 12:53
 ...
 I RANX="P" D  I RAQUIT G EXIT
 . ; ___ get procedure/CPT CODE(s)
 . D GETPROC
 . Q
 I RANX="I" D  I X G EXIT
 . ; ___ get imaging type ___ skip vascular lab
 . S X=$$GETIMG()
 . Q
 ...
 D START
 Q
START ; tell taskman to del task after running, set Radiology IO var
...
ASKIP ;
 S RANX=""
 S DIR(0)="S^I:Imaging Type;P:Procedure Name/CPT Code"
 S DIR("?")="Choose the next selection item."
 S DIR("A")="What do you want to choose next",DIR("B")="I"
...
GETPROC ;
 ...
 Q
ASKSORT ;
 S DIR(0)="S^CN:Case Number;CPT:CPT Code;DD:Date Desired;D:Days Wait;DO:Date of Order;DR:Date of Registration;I:Imaging Type;PN:Patient Name;PROC:Procedure Name"
...
GETDATA ;
 ...
 ..S RADTI="" F  S RADTI=$O(^RADPT("AR",RADTE,RADFN,RADTI)) Q:'RADTI  D FILTER1^RAPMW2 I 'RABAD D
 ...S RACNI=0 F  S RACNI=$O(^RADPT(RADFN,"DT",RADTI,"P",RACNI)) Q:'RACNI  D FILTER2^RAPMW2 I 'RABAD D CALC^RAPMW2
 ...Q
 ..Q
 .Q
 Q
EXIT ;
 D CLOSE^RAUTL ;close device
 K I,RA0,RA1,RA16,RA2,RA3,RA71REC,RA72
 ...
 K RAIO,RAIOM,RAIT,RAITYP,RAKEY,RALINE,RAMAXDT,RANEG,RANOW,RANX
 K RAOREC,RAORIEN,RAPATND,RAPATNM,RAPG,RAPOP,RAPROCNM,RAPSTX,RAQUIT
 K RAR,RAREC,RASAME,RASAME2,RASELDIV,RASINCE,RASORT,RASORTNM,RATDAYS
 K RATOTAL,RATYP,RAUTIL,RAWAITD,RATXT,RAXDT,RAXIT,RAXMST
 K POP,X,X1,X2,Y
 K ^TMP($J)
 ;Explanation of ^TMP nodes
 ; ^TMP($J,"RA I-TYPE","CT SCAN",ienFile79.2)=""
 ; ^TMP($J,"RA D-TYPE","SUPPORT ISC",ienFile79)=""
 ; ^TMP($J,"RA WAIT",ProcedureName,ienFile71)=""<--from EN1^RASELCT
 ; ^TMP($J,"RA WAIT1",ProcedureName)=CPTcode<--hdr of rpt, SETHD^RAPMW1
 ; ^TMP($J,"RA WAIT2",ienFile71)=""<--for screening cases, FILTER2^RAPMW2
 ;ex.   ^TMP($J,"RA WAIT","TEETH FULL MOUTH",31)=
 ;ex.   ^TMP($J,"RA WAIT1","TEETH FULL MOUTH")=70320
