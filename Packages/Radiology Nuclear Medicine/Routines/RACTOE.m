RACTOE ; GENERATED FROM 'RA ORDER EXAM' INPUT TEMPLATE(#1087), FILE 75.1;09/05/05
 D DE G BEGIN
...
X1 I '$D(RACAT)!('$D(RADFN))!('$D(RADIV))!('$D(RALIFN))!('$D(RAPIFN)) W !?3,*7,"Variables RACAT,RADFN,RADIV,RALIFN and RAPIFN must be defined!" S Y="@999"
 Q
...
X7 W !?3,*7,"A 'detailed' procedure or a 'series' of procedures is required!" S Y=""
 Q
8 S DQ=9 ;@2
...
X26 I $D(^RA(79,+RADIV,"HIS")) W !!?3,*7,^("HIS"),!
 Q
27 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=27 D X27 D:$D(DIEFIRE)#2 FIREREC^DIE17 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X27 I $O(^TMP($J,"RAWP",0)) S ^RAO(75.1,DA,"H",0)=^(0) F RAI=1:1 Q:'$D(^TMP($J,"RAWP",RAI,0))  S ^RAO(75.1,DA,"H",RAI,0)=^(0)
 Q
28 S DQ=29 ;@10
29 D:$D(DG)>9 F^DIE17 G ^RACTOE3
