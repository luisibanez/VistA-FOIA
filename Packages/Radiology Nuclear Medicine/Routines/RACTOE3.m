...
DE S DIE="^RAO(75.1,",DIC=DIE,DP=75.1,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^RAO(75.1,DA,""))=""
 I $D(^(0)) S %Z=^(0) S %=$P(%Z,U,4) S:%]"" DE(10)=%,DE(19)=% S %=$P(%Z,U,9) S:%]"" DE(26)=% S %=$P(%Z,U,14) S:%]"" DE(8)=% S %=$P(%Z,U,22) S:%]"" DE(31)=%
...
X7 I $O(^RAO(75.1,DA,"H",0)) D UPDT^RAUTL3("^RAO(75.1,"_DA_",""H"",")
 Q
8 S DW="0;14",DV="R*P200'X",DU="",DLB="REQUESTING PHYSICIAN",DIFLD=14
 S DU="VA(200,"
 S X=RAPIFN
 S Y=X
...
X30 I $D(RALIFN("OUT")) K RALIFN("OUT") S Y="@999"
 Q
31 S DW="0;22",DV="P44'a",DU="",DLB="REQUESTING LOCATION",DIFLD=22
 S DE(DW)="C31^RACTOE3"
 S DU="SC("
 S X=RALIFN
 S Y=X
...
X33 W !,"IS PATIENT SCHEDULED FOR PRE-OP" S %=2 D YN^DICN S:%<0 Y="@999" S:%=2 Y="@120" I '% W !!,"Enter 'YES' if patient is scheduled for pre-op, or 'NO' if not.",! S Y="@100"
 Q
34 D:$D(DG)>9 F^DIE17 G ^RACTOE6
