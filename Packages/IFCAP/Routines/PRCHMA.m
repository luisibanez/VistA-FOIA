...
LOOP D KILL^PRCHMA1 S PRCHNEW="",PRCHNORE=1,CAN=0
 ...
 ;
 ;W !! D GETPO^PRCHAMU G:'$G(PRCHPO)!$D(FIS) EXIT ;<<<< Orig. Stmt.
 S PRCFL=0
 W !! D GETPO^PRCHAMU
 ...
 ;D AMENDNO^PRCHAMU D DCLOCK^PRC0B("^PRC(442,"_PRCHPO_",") G:'$G(PRCHAM) EXIT ;>>>> Orig. Stmt.
...
CHK I '$$VERIFY^PRCHES5(PRCHPO) W !!,?5,"This purchase order has been tampered with.",!,?5,"Please notify IFCAP APPLICATION COORDINATOR." G EXIT
 ...
 ...S J=0 S J=$O(^PRC(443.6,PRCHPO,2,PRCH,1,J)) I J'>0 D:LCNT>END TOP W !!,?5,"Line item ",+$P(PRCHLN,U)," is missing it's description!",$C(7) S PRCHER="",LCNT=LCNT+2
 ...
 .D LIMIT^PRCHCD0
 ;
 ;I $G(PRCHAUTH)=1 S FILE=443.6 D LIMIT^PRCHCD0 I $G(ERROR) S PRCHER="" K FILE
