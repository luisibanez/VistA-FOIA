SROAPCA2 ;B'HAM ISC/MAM - PRINT OPERATIVE DATA ; [ 04/05/04  9:12 AM ]
 ;;3.0; Surgery ;**38,71,95,125**;24 Jun 93
 S SRA(206)=$G(^SRF(SRTN,206)),SRA(207)=$G(^SRF(SRTN,207)),SRA(209)=$G(^SRF(SRTN,209))
 ...
 S NYUK=$P($G(^SRF(SRTN,209.2)),"^") S SRAO("20H")=$S(NYUK="NS":"NS",1:NYUK)_"^491"
 ...
 S X=$P($G(^SRF(SRTN,207.1)),"^"),SRAO(28)=X_"^383.1"
 W !!,"VI. OPERATIVE DATA"
 ...
 W !,?2,"Number with IMA:",?33,$P(SRAO(2),"^"),?40,"Maze procedure:",?75,$P(SRAO(13),"^")
 ...
 W !,"Valve Repair:",?33,$P(SRAO(9),"^")
 W !,"LV Aneurysmectomy:",?33,$P(SRAO(10),"^")
 W !,"Other CT procedures (Specify): " I $P(SRAO(20),"^")'="" S SRQ=0 S X=$P(SRAO(20),"^") W:$L(X)<49 X,! I $L(X)>48 S Z=$L(X) D
 .I X'[" " W ?29,X Q
 ...
 W !,"Great vessel repair:",?33,$P(SRAO(21),"^")
 W !,"Foreign body removal:",?33,$P(SRAO(22),"^")
 W !,"Pericardiectomy:",?33,$P(SRAO(23),"^")
 W !,"Other Non-CT procedures-independently requiring CPB (Specify): " I $P(SRAO("20H"),"^")'="" S SRQ=0 S X=$P(SRAO("20H"),"^") W:$L(X)<18 X I $L(X)>17 S Z=$L(X) D
 .I X'[" " W X Q
 .S I=0,LINE=1 F  S SRL=$S(LINE=1:17,1:80) D  Q:SRQ
 ..I $E(X,1,SRL)'[" " W X,! S SRQ=1 Q
 ..S J=SRL-I,Y=$E(X,J),I=I+1 I Y=" " W $E(X,1,J-1),! S X=$E(X,J+1,Z),Z=$L(X),I=0,LINE=LINE+1 I Z<SRL W X S SRQ=1 Q
