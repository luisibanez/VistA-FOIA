SROACR1 ;BIR/MAM - OPERATIVE DATA, PAGE 1 ; [ 07/28/04  9:39 AM ]
 ;;3.0; Surgery ;**38,71,93,95,99,125**;24 Jun 93
 ;
 S SRA(206)=$G(^SRF(SRTN,206)),SRA(209)=$G(^SRF(SRTN,209))
 S SRA(207)=$G(^SRF(SRTN,207)) I $P(SRA(207),"^",27)="" K DA,DIE,DR S DA=SRTN,DIE=130,DR="469////5" D ^DIE K DA,DIE,DR S SRA(207)=$G(^SRF(SRTN,207))
 ...
 S NYUK=$P(SRA(209),"^",10) D YN S SRAO(13)=SHEMP_"^482"
 ...
 S NYUK=$P(SRA(207),"^",9) D YN S SRAO(19)=SHEMP_"^373",SRAO(20)=$P($G(^SRF(SRTN,209.1)),"^")_"^484"
 ;
DISP S SRPAGE="PAGE: 1 OF 2" D HDR^SROAUTL
 W "Cardiac surgical procedures with or without cardiopulmonary bypass "
 ;W ! F MOE=1:1:80 W "-"
 W !,"CABG distal anastomoses:"
 W !," 1. Number with vein:",?34,$P(SRAO(1),"^")
 W !," 2. Number with IMA:",?34,$P(SRAO(2),"^"),?40,"11. Bridge to transplant/Device:",?75,$P(SRAO(11),"^")
 W !," 3. Number with Radial Artery:",?34,$P(SRAO(3),"^"),?40,"12. TMR:",?75,$P(SRAO(12),"^")
 W !," 4. Number with Other Artery:",?34,$P(SRAO(4),"^"),?40,"13. Maze procedure:",?75,$P(SRAO(13),"^")
 W !," 5. Number with Other Conduit:",?34,$P(SRAO(5),"^"),?40,"14. ASD repair:",?75,$P(SRAO(14),"^")
 W !,?40,"15. VSD repair:",?75,$P(SRAO(15),"^")
 W !," 6. Aortic Valve Replacement:",?34,$P(SRAO(6),"^"),?40,"16. Myectomy for IHSS:",?75,$P(SRAO(16),"^")
 W !," 7. Mitral Valve Replacement:",?34,$P(SRAO(7),"^"),?40,"17. Myxoma resection:",?75,$P(SRAO(17),"^")
 W !," 8. Tricuspid Valve Replacement:",?34,$P(SRAO(8),"^"),?40,"18. Other tumor resection:",?75,$P(SRAO(18),"^")
 W !," 9. Valve Repair:  ",$J($P(SRAO(9),"^"),18),?40,"19. Cardiac transplant:",?75,$P(SRAO(19),"^")
 W !,"10. LV Aneurysmectomy:",?34,$P(SRAO(10),"^"),?40,"20. Other CT procedures:" S X=$P(SRAO(20),"^") W ?75,$S(X="NS":"NS",X="":"",1:"*"),!
 S X="IORVON;IORVOFF" D ENDR^%ZISS
 S X=$P(SRAO(20),"^") I X'="",X'="NS" S SRQ=0 W !,IORVON_"* Other CT Procedure, specify: " W:$L(X)<56 X,! I $L(X)>55 S Z=$L(X) D
 .I X'[" " W ?27,X Q
 ...
 W ! F MOE=1:1:80 W "-"
 Q
H491 S X=$P(SRAO(21),"^") I X'="",X'="NS" S SRQ=0 W !,IORVON_"** Other Non-CT Procedure (specify): " W:$L(X)<56 X,! I $L(X)>55 S Z=$L(X) D
 .I X'[" " W ?27,X Q
 .S I=0,LINE=1 F  S SRL=$S(LINE=1:39,1:80) D  Q:SRQ
 ..I $E(X,1,SRL)'[" " W X,! S SRQ=1 Q
 ..S J=SRL-I,Y=$E(X,J),I=I+1 I Y=" " W $E(X,1,J-1),! S X=$E(X,J+1,Z),Z=$L(X),I=0,LINE=LINE+1 I Z<SRL W X,! S SRQ=1 Q
...
MIS ; enter miscellaneous
 S DIE=130,DA=SRTN,DR="383T" D ^DIE K DR
 S SRAX=X,$P(^SRF(SRTN,207),"^",19)=SRAX
 I SRAX'="Y" Q
 K ^SRF(SRTN,207.1)
 S DIE=130,DA=SRTN,DR="383.1T" D ^DIE K DR Q
 Q
