SROCMPED ;B'HAM ISC/MAM - ENTER/EDIT OCCURRENCES ; [ 04/19/04  3:25 PM ]
 ;;3.0; Surgery ;**26,38,47,125**;24 Jun 93
...
START I '$D(^SRF(SRTN,SRTYPE,SRENTRY)) K SRENTRY S SRSOUT=0 Q
 ...
 I $G(SRNEW),$P(SR,"^",2)=7,SRTYPE=16,$P($G(^SRF(SRTN,"RA")),"^",2)="C" D REPEAT I SRSOUT G END
...
END K SRO,SR,X,DA,DIE,DR,Y
 Q
REPEAT D HDR^SROAUTL N SRREP S SRREP=""
 S Y=$P($G(^SRF(SRTN,209)),"^",12),C=$P(^DD(130,490,0),"^",2) D:Y'="" Y^DIQ S SRREP=Y
 K DIR S DIR("A",1)="  Was this patient placed on ventilator support more than once for",DIR("A",2)="  any reason within 30 days after the operation, AND was this repeat"
 S DIR("A",3)="  ventilator support related to the index operation AND did it occur",DIR("A")="  during the same admission in-hospital as the operation (Y/N)"
 S DIR(0)="Y" S:SRREP'="" DIR("B")=SRREP D ^DIR K DIR I $D(DTOUT)!$D(DUOUT) S SRSOUT=1 Q
 S SRREP=Y(0) K DA,DIE,DR S DA=SRTN,DIE=130,DR="490///"_SRREP D ^DIE K DA,DIE,DR
 S SRNEW=0
 Q
