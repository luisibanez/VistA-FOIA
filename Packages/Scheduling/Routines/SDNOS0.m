SDNOS0 ;ALB/LDB - NO SHOW REPORT ; 07 May 99 10:21 AM
 ;;5.3;Scheduling;**20,194,410**;Aug 13, 1993
 D END1^SDNOS
...
SDED S SDBEG=0,SDEN=0 I $D(SDED),(SDBEG1>(SDED+.99999)) S SDEN=1 Q
 ...
 Q
 ;
CHK S SDAPP=0 F SDZZ3=1:1 S SDAPP=$O(^SC(SDCL,"S",SDBEG1,1,SDAPP)) Q:SDAPP'>0  I $D(^SC(SDCL,"S",SDBEG1,1,SDAPP))=10,$P(^(SDAPP,0),U,9)'="C" S SDPAT=$P(^SC(SDCL,"S",SDBEG1,1,SDAPP,0),U,1) I $D(^DPT(SDPAT,"S",SDBEG1)) D CHK1
 Q
 ;
