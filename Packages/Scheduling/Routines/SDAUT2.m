SDAUT2 ;MAN/GRR - LOOK FOR OPEN SLOTS ; 07 Jan 2000 11:57 AM
 ;;5.3;Scheduling;**206,168,186**;Aug 13, 1993
 K SDNOSH
EN1 S (FND,DUPE)=0,NDATE="",SDSOH=$S('$D(^SC(SC,"SL")):0,$P(^("SL"),"^",8)']"":0,1:1) I $S('$D(^DPT(+A,.35)):0,$P(^(.35),"^",1)']"":0,1:1) S MESS="NOT REBOOKED, PATIENT HAS DIED" G END
 ...
 S $P(^SC(SC,"S",NDATE,1,Y,0),"^",10)=$P(SDNODE,"^",10)
 I $D(^SC("ARAD",SC,GDATE,+A)) S ^SC("ARAD",SC,NDATE,+A)=""
 S SDTTM=NDATE,SDPL=Y,SDSC=SC,DFN=+A,SDRT="A" D RT^SDUTL,EVT
