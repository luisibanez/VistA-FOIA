ONCOCFR ;Hines OIFO/RTK - CASEFINDING RADIOLOGY ;7/21/93
 ;;2.11;ONCOLOGY;**13,24,25,26,27,34,37,39**;Mar 07, 1995
 ;
...
T ;Start Date/Go to Date
 ...
 K DIR
 S DIR("A")="          Go to Date",DIR(0)="D" D ^DIR
 G EX:Y=""!(Y[U)
...
SER ;SEARCH RADIOLOGY
 ...
 F  S XDT=$O(^RADPT("AR",XDT)) Q:XDT=""!(XDT>XED)  S D0=$O(^(XDT,0)),D1=$O(^(D0,0)) D
 ...
 ..I MG["MALIG" S RA($P(^RADPT(D0,0),U))=$P(XDT,".")_U_$P(RA0,U,2)_U_D1
...
N2 ;
 ; Create the new suspense multiple entry for the patient
 K DD,DO
 ...
 ;
EX ;EXIT
 K ONCO,ED,HT,MG,OSP,RA,RA0,RASDC0,RAO,SD,WED,WSD,XX,DIJ,DP,D,D0
 K %T,%ZISOS
 K AFFDIV,DVMTCH,INST,ONCDIVSP,RE1,Z,^TMP("ONCO",$J) D ^%ZISC
 Q
DIV ;
