ONCACD0 ;Hines OIFO/GWB - Extract driver ;06/11/01
 ;;2.11;Oncology;**9,12,20,24,25,28,29,30,36,37,38,40,41,44,45**;Mar 07, 1995
 ;
...
EN2 N EXTRACT,HDRIEN,STAT,STAT1,STAT2,DATE,YESNO,BDT,SDT,EDT,QUEUE
 ...
 I STEXT=0 S EXTRACT=$O(^ONCO(160.16,"B","NCDB EXTRACT V11",0))
...
GETREC(EXTRACT,OUT) ;Select record layout
 ...
 W !,"  1) VACCR Record Layout v11 (VA Registry)"
 W !,"  2) NAACCR State Record Layout v11"
 W !
 N DIR,X,Y
 S DIR(0)="SAO^1:VACCR Record Layout v11;2:NAACCR State Record Layout v11"
 S DIR("A")=" Select record layout: "
 S DIR("?")="Select the record layout to use"
 D ^DIR
 I $D(DIRUT) S OUT=1 Q
 I +Y<1 S OUT=1 Q
 I Y=1 S EXT="VACCR",EXTRACT=$O(^ONCO(160.16,"B","VACCR EXTRACT V11",0))
 I Y=2 S EXT="STATE",EXTRACT=$O(^ONCO(160.16,"B","STATE EXTRACT V11",0))
 S HDRIEN=EXTRACT
 Q
 ;
...
GETDT(SDT,EDT,DATE,OUT) ; Select a date range
 ...
 S DIR("?")=" "
 S DIR("?",1)="   Enter the DATE CASE COMPLETED date of the"
...
VERIFY(STAT,DATE,SDT,EDT,STEXT,YESNO,OUT) ;Verify settings
 N DIR,Y
 I STEXT=0 S RL=$P(^ONCO(160.16,HDRIEN,0),U,1)
 I STEXT=1 S RL=$P(^ONCO(160.16,HDRIEN,0),U,1)
