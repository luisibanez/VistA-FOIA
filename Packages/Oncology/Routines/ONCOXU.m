ONCOXU ; GENERATED FROM 'ONCO XABSTRACT RECORD' PRINT TEMPLATE (#826) ; 08/13/05 ; (FILE 165.5, MARGIN=80)
 G BEGIN
N W !
...
BEGIN ;
 ...
 D N:$X>32 Q:'DN  W ?32 S X=DT S Y=X K DIP K:DN Y S Y=X D DT
 W ?52 W !!,"" K DIP K:DN Y
...
A1 ;
 ...
 S X=$G(^ONCO(160,D0,0)) D N:$X>10 Q:'DN  W ?10 S Y=$P(X,U,1) S C=$P(^DD(160,.01,0),U,2) D Y^DIQ:Y S C="," W $E(Y,1,30)
 D N:$X>39 Q:'DN  W ?39 W "SSN: "
 D SSN^ONCOES W $J(X,12) K Y(160,2)
 D N:$X>2 Q:'DN  W ?2 W "Sex: "
 S X=$G(^ONCO(160,D0,0)) D N:$X>10 Q:'DN  W ?10 S Y=$P(X,U,8) W:Y]"" $S($D(DXS(2,Y)):DXS(2,Y),1:Y)
 D N:$X>39 Q:'DN  W ?39 W "DOB: "
 D DOB^ONCOES W $J(X,11) K Y(160,3)
 D N:$X>2 Q:'DN  W ?2 W "Race: "
 S X=$G(^ONCO(160,D0,0)) D N:$X>10 Q:'DN  W ?10 S Y=$P(X,U,6) S Y=$S(Y="":Y,$D(^ONCO(164.46,Y,0))#2:$P(^(0),U),1:Y) W $E(Y,1,50)
 D N:$X>2 Q:'DN  W ?2 W "Status: "
 S X=$G(^ONCO(160,D0,1)) D N:$X>10 Q:'DN  W ?10 S Y=$P(X,U,1) W:Y]"" $S($D(DXS(3,Y)):DXS(3,Y),1:Y)
 W ?10 W !," " K DIP K:DN Y
 W ?21 D DLC^ONCOCRF,DATEOT^ONCOES K DIP K:DN Y
 D N:$X>2 Q:'DN  W ?2 W "Date of Last Contact or Death:"
 W ?34 W X K DIP K:DN Y
 D N:$X>2 Q:'DN  W ?2 W "Autopsy Date/Time: "
 S X=$G(^ONCO(160,D0,1)) S Y=$P(X,U,9) S Y(0)=Y S X=Y D DATEOT^ONCOES W $E(Y,1,30)
 D N:$X>39 Q:'DN  W ?39 W "Autopsy #: "
 S X=$G(^ONCO(160,D0,1)) W ?0,$E($P(X,U,10),1,15)
 D N:$X>2 Q:'DN  W ?2 W "Cause of Death: "
 S Y=$P(X,U,3) S Y=$S(Y="":Y,$D(^ICD9(Y,0))#2:$P(^(0),U),1:Y) W $E(Y,1,7)
 Q
A1R ;
 K J(100),I(100) S:$D(I(0,0)) D0=I(0,0)
 W ?20 W !!," " K DIP K:DN Y
 D N:$X>2 Q:'DN  W ?2 W "Abstract Status: "
 S X=$G(^ONCO(165.5,D0,7)) S Y=$P(X,U,2) W:Y]"" $S($D(DXS(4,Y)):DXS(4,Y),1:Y)
