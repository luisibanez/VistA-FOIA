ONCOCFL1 ;Hines OIFO/GWB - LAB CASEFINDING  ;8/11/93
 ;;2.11;ONCOLOGY;**25,26,27,28,29,32,33,43,44**;Mar 07, 1995
 ;
...
SER ;Search LAB DATA (63) file
 ...
 S LRDFN=0 F  S LRDFN=$O(^TMP($J,LRDFN)) G RPT:LRDFN="" S LRSDT=$O(^(LRDFN,0)),LD=^(LRSDT),LRSS=$P(LD,U),LRI=$P(LD,U,6),XDT=$S(LRSS="AU":$P(^LR(LRDFN,LRSS),U),1:$P(^LR(LRDFN,LRSS,LRI,0),U)) D CK
 ;
CK ;Check ONCOLOGY PATIENT (160) file
 ...
 I XD0="" S D="B",DIC="^ONCO(160,",DIC(0)="Z" D FILE^DICN S ONCIEN=+Y D SET Q
...
DIV ;Check division
 S DVMTCH=1,INST=""
 I LRSS="AU" D
 .S LBACC=$P($G(^LR(LRDFN,LRSS)),"^",6)
 .S LBYEAR=$P($G(^LR(LRDFN,LRSS)),"^",1)
 I LRSS'="AU" D
 .S LBACC=$P($G(^LR(LRDFN,LRSS,LRI,0)),"^",6)
 .S LBYEAR=$P($G(^LR(LRDFN,LRSS,LRI,0)),"^",1)
 I (LBACC="")!(LBYEAR="") Q
