...
SSNNM(DFN) ; SSN and name on first line of screen
 ...
 N X,SSN,HRN
 S X=$S($D(^DPT(+DFN,0)):^(0),1:""),SSN=$P(X,"^",9)
 I SSN S SSN=$E(SSN,1,3)_"-"_$E(SSN,4,5)_"-"_$E(SSN,6,10)
 ;
ID S SSN=$$ID^DGLBPID(DFN) ;**GFT/VW
 ; for IHS or EHR, replace SSN with HRN
 I $G(DUZ("AG"))'="V" D
 . Q:SSN]""
 . S HRN=$$HRN^AUPNPAT3(+DFN,DUZ(2))
 . I HRN S SSN="HRN-"_HRN
